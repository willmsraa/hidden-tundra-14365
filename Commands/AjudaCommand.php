 ...
 */
class AjudaCommand extends UserCommand
{
    /**
     * @var string
     */
    protected $name = 'ajuda';

    /**
     * @var string
     */
    protected $description = 'Ajuda com comandos';

    /**
     * @var string
     */
    protected $usage = '/ajuda or /ajuda <comando>';
    ...
    public function execute()
    {
        ...
        if ($command_str === '') {
            $data['text'] = '*Lista de Comandos*:' . PHP_EOL;
            ...
            if (count($admin_commands) > 0) {
                $data['text'] .= PHP_EOL . '*Lista de Comandos Admin*:' . PHP_EOL;
                foreach ($admin_commands as $admin_command) {
                    $data['text'] .= '/' . $admin_command->getName() . ' - ' . $admin_command->getDescription() . PHP_EOL;
                }
            }

            $data['text'] .= PHP_EOL . 'Para ajuda exata de um comando digite: /ajuda <comando>';
        ...
        $data['text'] = 'Nenhuma ajuda encontrada para o Comando /' . $command_str . ' não encontrado';
